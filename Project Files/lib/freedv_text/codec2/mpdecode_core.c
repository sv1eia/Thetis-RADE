...
/* Thetis-RADE patch (Christos Nikolaou / SV1EIA, 2026,
 * sv1eia@gmail.com): MSVC has no C99 variable-length arrays.  Use
 * _alloca for the runtime-sized scratch buffers in Demod2D / Somap /
 * symbols_to_llrs (the only VLAs in this file).  Stack allocation is
 * fine -- they hold at most a few-hundred floats. */
#ifdef _MSC_VER
#  include <malloc.h>
#  define rade_alloca(n) _alloca(n)
#else
#  include <alloca.h>
#  define rade_alloca(n)  alloca(n)
#endif
...
void Somap(float bit_likelihood[],    /* number_bits, bps*number_symbols */
           ...
           int number_symbols) {
  int n, i, j, k, mask;
  float* num = (float*)rade_alloca(sizeof(float) * bps);
  float* den = (float*)rade_alloca(sizeof(float) * bps);
  float metric;
...
void symbols_to_llrs(float llr[], COMP rx_qpsk_symbols[], float rx_amps[],
  ...
  float* symbol_likelihood = (float*)rade_alloca(sizeof(float) * nsyms * QPSK_CONSTELLATION_SIZE);
  float* bit_likelihood    = (float*)rade_alloca(sizeof(float) * nsyms * QPSK_BITS_PER_SYMBOL);
...
void fsk_rx_filt_to_llrs(float llr[], float rx_filt[], float v_est,
  ...
  int i;
  int bps = log2(M);
  float* symbol_likelihood = (float*)rade_alloca(sizeof(float) * M * nsyms);
  float* bit_likelihood    = (float*)rade_alloca(sizeof(float) * bps * nsyms);
